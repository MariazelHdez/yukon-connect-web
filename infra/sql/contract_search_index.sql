...
-- Semantic search preparation. pgvector may not be installed in every local or CI
-- database, so extension and vector-dependent objects are created only when
-- the extension is available. Mock embeddings currently use 384 dimensions.
do $$
begin
  if exists (select 1 from pg_available_extensions where name = 'vector') then
    create extension if not exists vector;
  else
    raise notice 'pgvector extension is not available; skipping contract_search_index.embedding setup.';
  end if;
exception
  when insufficient_privilege then
    raise notice 'Insufficient privileges to create pgvector extension; skipping contract_search_index.embedding setup.';
end;
$$;
...
begin
  if to_regtype('vector') is not null then
    alter table contract_search_index
      add column if not exists embedding vector(384);

    comment on column contract_search_index.embedding is
      'Semantic embedding for search_text. Development data is generated by MockEmbeddingProvider; replace with a real EmbeddingProvider implementation for production embeddings.';
  end if;
end;
$$;
